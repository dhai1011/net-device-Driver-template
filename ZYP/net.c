...
int astonnet_init(struct net_device *dev)
{
    ...
    ether_setup(dev);//填充一些以太网中的设备结构体的项（hai：内核提供的函数）
...
{
	int result=0;
    //astonnet_devs = alloc_etherdev(sizeof(struct astonnet_priv ));
	astonnet_devs = alloc_etherdev(sizeof(struct net_device));  /*hai：默认网卡驱动名称为 "eth%d"*/
	astonnet_devs->netdev_ops = &astonnet_netdev_ops;

	strcpy(astonnet_devs->name, "astonnet0");/*hai：再次修改了网卡驱动名称*/
